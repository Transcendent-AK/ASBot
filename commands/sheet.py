...
from utils.masterlist_ops import add_player_to_guild, remove_player_from_guild, edit_player_in_guild
from utils.watchlist_ops import add_player_to_banlist, remove_player_from_banlist, edit_player_in_banlist

# Modal classes defined outside the function
class AddPlayerModal(discord.ui.Modal, title="Add Player to Masterlist"):
    ...
    rank = discord.ui.TextInput(
        label="Rank",
        placeholder="Enter rank (e.g., 5 - Lost Soul)",
        required=True,
        max_length=50
    )
    status = discord.ui.TextInput(
        label="Status",
        placeholder="Enter status (e.g., Active Main, Inactive, etc.)",
        required=True,
        max_length=50
    )
    ...
    sus_alert = discord.ui.TextInput(
        ...
        placeholder="Enter 'Yes' or 'No' for suspicious alert",
        required=True,
        max_length=3
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            # Validate date format if not 'n/a'
            ...
            # Validate sus_alert
            if self.sus_alert.value.lower() not in ['yes', 'no']:
                await interaction.response.send_message("❌ Suspicious Alert must be 'Yes' or 'No'", ephemeral=True)
                return
            ...
            row_data = [
                ...
                self.rank.value,
                self.status.value,
                self.known_alts.value or "",  # Empty string if not provided
                self.house.value or "",       # Empty string if not provided
                self.discord_id.value or "",  # Empty string if not provided
                self.notes.value or "",       # Empty string if not provided
                self.sus_alert.value.title()  # Capitalize Yes/No
            ]
...
class EditPlayerModal(discord.ui.Modal, title="Edit Player in Masterlist"):
    player_ign = discord.ui.TextInput(
    ...
    rank = discord.ui.TextInput(
        label="New Rank",
        placeholder="Enter new rank (e.g., 5 - Lost Soul)",
        required=True,
        max_length=50
    )
    status = discord.ui.TextInput(
        label="New Status",
        placeholder="Enter new status (e.g., Active Main, Inactive, etc.)",
        required=True,
        max_length=50
    )
    discord_id = discord.ui.TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Prepare row data with essential fields (join date and other fields will be preserved from existing data)
            row_data = [
                self.player_ign.value,
                "",  # Join Date (will be preserved from existing data)
                self.rank.value,
                self.status.value,
                "",  # Known Alts (will be preserved from existing data)
                "",  # House (will be preserved from existing data)
                self.discord_id.value or "",  # Discord ID
                "",  # Notes (will be preserved from existing data)
                self.sus_alert.value.title()  # Capitalize Yes/No
            ]
            
            success, message = edit_player_in_guild(self.player_ign.value, row_data, interaction.user.name)
            await interaction.response.send_message(message, ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"❌ Error: {str(e)}", ephemeral=True)
...
class AddToWatchlistModal(discord.ui.Modal, title="Add Player to Watchlist"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Prepare row data with all Watchlist fields
            row_data = [
                self.player_ign.value,
                self.status.value,
                self.guild.value,
                ...
                self.other_notes.value or "",  # Empty string if not provided
                self.screenshots.value or "",  # Empty string if not provided
                self.known_alts.value or "",   # Empty string if not provided
                self.discord_id.value or "",   # Empty string if not provided
                self.house_name.value or ""    # Empty string if not provided
            ]
...
class CustomDateModal(discord.ui.Modal, title="Enter Custom Date"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Store the custom date and show the modal
            await interaction.response.send_modal(AddPlayerModalWithDate(self.custom_date.value))
        except Exception as e:
            await interaction.response.send_message(f"❌ Error: {str(e)}", ephemeral=True)
    ...
    async def create_sheet_menu(interaction: discord.Interaction):
        class DateSelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                # Store the selected date and show the modal
                await interaction.response.send_modal(AddPlayerModalWithDate(self.values[0]))
        ...
        class DatePickerView(discord.ui.View):
            ...
            @discord.ui.button(label="📅 Custom Date", style=discord.ButtonStyle.secondary)
            async def custom_date_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                await interaction.response.send_modal(CustomDateModal())
        ...
        class StatusSelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                # Store the selected status and show the date picker
                await interaction.response.edit_message(
                    content="Select join date:",
                    view=DatePickerView()
                )

        class AddPlayerModalWithDate(discord.ui.Modal, title="Add Player to Masterlist"):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                try:
                    ...
                    # Prepare row data with essential fields (others will be empty)
                    row_data = [
                        self.player_ign.value,
                        self.selected_date,  # Use the date from the date picker
                        self.rank.value,
                        self.status.value,
                        "",  # Known Alts (empty)
                        "",  # House (empty)
        ...
        class WatchlistStatusSelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                # Store the selected status and show the reason select
        ...
        class WatchlistReasonSelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                # Store the selected reason and show the action by select
                view = discord.ui.View()
        ...
        class ActionBySelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                # Store the selected action by and show the modal
        ...
        class AddToWatchlistModalWithActionBy(discord.ui.Modal, title="Add Player to Watchlist"):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                try:
                    ...
                    row_data = [
                        self.player_ign.value,
                        self.selected_status,  # From dropdown
                        self.guild.value or "",  # Guild (can be empty)
                        self.status_date.value,
                        self.selected_reason,  # From dropdown
                        self.selected_action_by,  # From dropdown
                        self.other_notes.value or "",  # Empty string if not provided
                        "",  # Screenshots (empty)
                        "",  # Known Alts (empty)
                        "",  # Discord ID (empty)
        ...
        class PersistentActionView(discord.ui.View):
            ...
            @discord.ui.button(label="Add Player to Masterlist", style=discord.ButtonStyle.success, custom_id="persistent_add")
            ...
            @discord.ui.button(label="Remove Player from Masterlist", style=discord.ButtonStyle.danger, custom_id="persistent_remove")
            ...
            @discord.ui.button(label="Edit Player in Masterlist", style=discord.ButtonStyle.secondary, custom_id="persistent_edit")
            ...
            @discord.ui.button(label="Add to Watchlist", style=discord.ButtonStyle.danger, custom_id="persistent_watchlist_add")
        ...
        embed.add_field(name="🎯 Masterlist", value="Add, remove, or edit players in the main list", inline=False)
        ...
        await interaction.response.send_message(
            ...
            ephemeral=False  # This makes it visible to everyone
        )